...
function mainAnimation(){
	...
	$(window).on('resize', function(){
        var tops = $('.main_list_wrap .swiper-wrapper .slide_img').eq(0).outerHeight() + $('.main_list_wrap .swiper-wrapper .slide_infomation').eq(0).outerHeight();
		document.styleSheets[0].addRule('div.main_list:after','content: "";position: absolute;\n' +
			'    left: 50%;\n' +
			'    top: '+tops+'px;\n' +
			'    bottom: auto;\n' +
			'    display: block;\n' +
...
$(window).load(function(){
	var i, r = 10, speed = 0.3, tlm = new TimelineMax();

    $(document).on('mouseover', '.icon_search', searchOverHandler3).on('mouseout', '.icon_search', searchOutHandler3);

    TweenMax.set($('.icon_search_box'), {scale:0.5});
    tlm.to($('.icon_search_box'), 3, {rotation:-360, repeat:-1, ease:Power0.easeNone},0);
    tlm.staggerTo($('.icon_search_box span'), 0.5,{
    	cycle:{
			x:function(index, target){
                return r * Math.cos((360/6*index) * Math.PI/180);
			},
			y:function(index, target){
                return r * Math.sin((360/6*index) * Math.PI/180);
			}
		}, scale:Math.random()+1, yoyo:true, repeat:-1}, 0.5);

    tlm.seek(6);
    tlm.pause();

    function searchOverHandler3()
    {
        if(window.innerWidth > 1024) {
            TweenMax.to($('.icon_search'),speed,{x:-6, y:-3, ease:Cubic.easeOut});
            if($('.gnb_btn_area').hasClass('ent3'))
			{
                TweenMax.to($('.icon_search_in .ico1'),speed,{borderWidth:2, width:28, height:28, ease:Cubic.easeOut});
                TweenMax.to($('.icon_search_in .ico2'),speed,{width:2, height:10, left:26, top:22, ease:Cubic.easeOut});
			}else{
                TweenMax.to($('.icon_search_in .ico1'),speed,{borderWidth:1, width:28, height:28, ease:Cubic.easeOut});
                TweenMax.to($('.icon_search_in .ico2'),speed,{width:1, height:10, left:26, top:22, ease:Cubic.easeOut});
			}
            TweenMax.to($('.icon_search_box'),speed,{opacity:1, scale:1, left:14, top:14, ease:Cubic.easeOut});
        }
        tlm.play();
    }
    function searchOutHandler3()
    {
        if(window.innerWidth > 1024) {
            TweenMax.to($('.icon_search'), speed, {x: 0, y: 0, ease: Cubic.easeOut});
            TweenMax.to($('.icon_search_in .ico1'), speed, {borderWidth: 2, width: 20, height: 20, ease: Cubic.easeOut});
            TweenMax.to($('.icon_search_in .ico2'), speed, {width: 2, height:8, left: 19, top: 16, ease: Cubic.easeOut});
            TweenMax.to($('.icon_search_box'), speed, {opacity:0, scale:0.5, top:10, left:10, ease:Cubic.easeOut});
        }
        tlm.pause();
    }
