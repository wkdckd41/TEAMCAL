 ...
 */
/*(function(factory){'use strict';if(typeof define==='function'&&define.amd){define(['jquery'],factory)}else{factory(jQuery)}}(function($){'use strict';var namespace='ellipsis',span='<em style="white-space: nowrap;">',defaults={lines:'auto',ellipClass:'ellip',responsive:false};function Ellipsis(el,opts){var base=this,currLine=0,lines=[],setStartEllipAt,startEllipAt,resizeTimer,currOffset,lineHeight,contHeight,words,htmlEntities;htmlEntities={'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#x27;','`':'&#x60;','/':'&#47;'};base.$cont=$(el);base.opts=$.extend({},defaults,opts);function create(){base.text=String(base.$cont.html());base.opts.ellipLineClass=base.opts.ellipClass+'-line';base.$el=$('<em class="'+base.opts.ellipClass+'" />');base.$el.text(base.text);base.$cont.empty().append(base.$el);init()}function init(){if(typeof base.opts.lines==='number'&&base.opts.lines<2){base.$el.addClass(base.opts.ellipLineClass);return}contHeight=base.$cont.height();if(base.opts.lines==='auto'&&base.$el.prop('scrollHeight')<=contHeight){return}if(!setStartEllipAt){return}words=$.trim(base.text);words=words.replace(/ class/gi,'class');words=words.replace(/ style/gi,'style');words=words.replace(/ id/gi,'id');words=words.replace(/ :/gi,':');words=words.replace(/: /gi,':');words=words.replace(/ ;/gi,';');words=words.replace(/; /gi,';');words=words.replace(/ =/gi,'=');words=words.replace(/= /gi,'=');words=words.split(/\s+/);$.each(words,function(i){words[i]=words[i].replace(/class/gi,' class');words[i]=words[i].replace(/style/gi,' style');words[i]=words[i].replace(/id/gi,' id')});base.$el.html(span+words.join('</em> '+span)+'</em>');base.$el.find('em').each(setStartEllipAt);if(startEllipAt!=null){updateText(startEllipAt)}}function updateText(nth){if(words[nth]==undefined){}else{words[nth]='<em class="'+base.opts.ellipLineClass+'">'+words[nth];words.push('</em>');base.$el.html(words.join(' '))}}function escapeText(text){return String(text).replace(/[&<>"'\/]/g,function(s){return htmlEntities[s]})}if(base.opts.lines==='auto'){var setStartEllipByHeight=function(i,word){var $word=$(word),top=$word.position().top;lineHeight=lineHeight||$word.height();if(top===currOffset){lines[currLine].push($word)}else{currOffset=top;currLine+=1;lines[currLine]=[$word]}if(top+lineHeight>contHeight){startEllipAt=i-lines[currLine-1].length;return false}};setStartEllipAt=setStartEllipByHeight}if(typeof base.opts.lines==='number'&&base.opts.lines>1){var setStartEllipByLine=function(i,word){var $word=$(word),top=$word.position().top;if(top!==currOffset){currOffset=top;currLine+=1}if(currLine===base.opts.lines){startEllipAt=i;return false}};setStartEllipAt=setStartEllipByLine}if(base.opts.responsive){var resize=function(){lines=[];currLine=0;currOffset=null;startEllipAt=null;base.$el.html(escapeText(base.text));init();clearTimeout(resizeTimer);resizeTimer=setTimeout(init,100)};$(window).on('resize.'+namespace,resize)}create()}$.fn[namespace]=function(opts){return this.each(function(){try{$(this).data(namespace,(new Ellipsis(this,opts)))}catch(err){if(window.console){console.error(namespace+': '+err)}}})}}));*/
...
(function(factory) {
    ...
    function Ellipsis(el, opts) {
        var base = this,
            currLine = 0,
            lines = [],
            setStartEllipAt, startEllipAt, startEmptyAt, resizeTimer, currOffset, lineHeight, contHeight, words, htmlEntities;
        htmlEntities = {
            '&': '&amp;',
            '<': '&lt;',
            '>': '&gt;',
        ...
        function create() {
            base.text = String(base.$cont.html());
            base.opts.ellipLineClass = base.opts.ellipClass + '-line';
            base.$el = $('<em class="' + base.opts.ellipClass + '" />');
        ...
        function init() {
            if (typeof base.opts.lines === 'number' && base.opts.lines < 2) {
                base.$el.text(base.$el.text().replace(/&amp;/gi, '&'));
                base.$el.text(base.$el.text().replace(/&#47;/gi, '/'));
            ...
            words = $.trim(base.text);
            words = words.replace(/ class/gi, 'class');
            words = words.replace(/ style/gi, 'style');
            words = words.replace(/ id/gi, 'id');
            ...
            $.each(words, function(i) {
                words[i] = words[i].replace(/class/gi, ' class');
                words[i] = words[i].replace(/style/gi, ' style');
                words[i] = words[i].replace(/id/gi, ' id');
                words[i] = words[i].replace(/\s+/gi, 'ª')
            });
            words = words.join('').replace(/<emclass="main_blue">/gi, '＠');
            words = words.replace(/<\/em>/gi, '§');
            words = words.split('');

            base.$el.html(span + words.join('</em> ' + span) + '</em>');
            base.$el.find('em').each(function(i, word){
                if($(word).text() == '＠' || $(word).text() == '§' )
                {
                    $(word).addClass('notcopy');
                }
            });
            base.$el.find('em').each(setStartEllipAt);

            updateText(startEllipAt)
        }

        function updateText(nth) {
            if (words[nth] == undefined) {
                words = words.join('').replace(/＠/gi, '<em class="main_blue">');
                words = words.replace(/§/gi, '</em>');
                words = words.replace(/ª/gi, ' ');
                base.$el.html(words)
            } else {
                var arrtxt = words.join('');
                var txtlen = arrtxt.match(/＠/gi);
                if(txtlen == null) {
                    words[nth] = '<em class="' + base.opts.ellipLineClass + '">' + words[nth];
                    words = words.join('');
                    words = words.replace(/＠/gi, '<em class="main_blue">');
                    words = words.replace(/§/gi, '</em>');
                    words = words.replace(/ª/gi, ' ');
                    words += '</em>';
                    base.$el.html(words)
                }else{
                    var beforetxt = arrtxt.substring(0, nth).split('');
                    var aftertxt = arrtxt.substring(nth, Number.MAX_VALUE).split('');
                    var n=nth-1;
                    while (n>=0) {
                        if(beforetxt[n] == '§') {
                            words[nth] = '<em class="' + base.opts.ellipLineClass + '">' + words[nth];
                        ...
                        if(beforetxt[n] == '＠') {
                            beforetxt[n] = '<em class="' + base.opts.ellipLineClass + '">' + beforetxt[n];
                            words = beforetxt.concat(aftertxt).join('');
        ...
        if (typeof base.opts.lines === 'number' && base.opts.lines > 1) {
            var setStartEllipByLine = function(i, word) {
                var $word = $(word), top = $word.position().top;
                if (top < 1) {
                    top = 0;
                } else {
                    top = Math.floor(top);
                ...
                if (currLine === base.opts.lines) {
                    startEmptyAt = words.join('').substr(0, i);
                    startEllipAt = i
                    return false
                }
            };
